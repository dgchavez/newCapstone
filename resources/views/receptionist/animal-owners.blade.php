<x-app-layout>
    ...
    <!-- Receptionist Approve Owner Modal -->
    <div id="receptionistApproveModal" class="fixed inset-0 bg-gray-900/70 backdrop-blur-sm flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-lg shadow-xl p-6 max-w-md w-full">
            <div class="text-center mb-5">
                ...
                <h3 class="text-xl font-bold text-gray-800">Approve Owner</h3>
                <p class="text-sm text-gray-600 mt-2">Are you sure you want to approve this owner? This will activate their account.</p>
            </div>
            
            <div class="flex space-x-3 justify-center">
                <button id="confirmReceptionistApproveBtn" type="button" class="bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-4 rounded-lg">
                    Approve
                </button>
                <button id="cancelReceptionistApproveBtn" type="button" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-medium py-2 px-4 rounded-lg">
                    Cancel
                </button>
            </div>
        </div>
    ...
    <script>
        // Current approval form that will be submitted
        ...
        function confirmApproval(userId) {
            ...
            currentApprovalForm = buttonElement.closest('form');
            
            // Show modal
            const modal = document.getElementById('receptionistApproveModal');
            if (modal) {
                modal.classList.remove('hidden');
                
                // Set up confirm button
                const confirmBtn = document.getElementById('confirmReceptionistApproveBtn');
                if (confirmBtn) {
                ...
                const cancelBtn = document.getElementById('cancelReceptionistApproveBtn');
                if (cancelBtn) {
                    // Remove existing event listeners
        ...
        // Make sure the modal is set up when the page loads
    ...
    <!-- Receptionist Reset Password Modal -->
    <div id="receptionistResetModal" class="fixed inset-0 bg-gray-900/70 backdrop-blur-sm flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-lg shadow-xl p-6 max-w-md w-full">
            <div class="text-center mb-5">
            ...
            <div class="flex space-x-3 justify-center">
                <button id="confirmReceptionistResetBtn" type="button" class="bg-yellow-600 hover:bg-yellow-700 text-white font-medium py-2 px-4 rounded-lg">
                    Reset Password
                </button>
                <button id="cancelReceptionistResetBtn" type="button" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-medium py-2 px-4 rounded-lg">
                    Cancel
                </button>
            </div>
        </div>
    ...
    <script>
        // Current reset form that will be submitted
        ...
        function confirmReset() {
            ...
            // Find the form that triggered this
            currentResetForm = event.target;
            
            // Show modal
            const modal = document.getElementById('receptionistResetModal');
            if (modal) {
                modal.classList.remove('hidden');
                
                // Set up confirm button
                const confirmBtn = document.getElementById('confirmReceptionistResetBtn');
                if (confirmBtn) {
                    // Remove existing event listeners
                    const newConfirmBtn = confirmBtn.cloneNode(true);
                    confirmBtn.parentNode.replaceChild(newConfirmBtn, confirmBtn);
                ...
                // Set up cancel button
                const cancelBtn = document.getElementById('cancelReceptionistResetBtn');
                if (cancelBtn) {
                    // Remove existing event listeners
                    const newCancelBtn = cancelBtn.cloneNode(true);
                    cancelBtn.parentNode.replaceChild(newCancelBtn, cancelBtn);
        ...
        // Update your DOMContentLoaded handler to include this modal too
        document.addEventListener('DOMContentLoaded', function() {
            // Previous modal handlers...
            
            // Reset password modal handler
            const resetModal = document.getElementById('receptionistResetModal');
            if (resetModal) {
